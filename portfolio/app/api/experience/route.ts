...
import { experienceSchema, paginationSchema } from '@/lib/validation/schemas';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const items = await db
      .select()
      .from(experience)
      .orderBy(asc(experience.order_index))
      .limit(limit)
      .offset(from);
    const total = await db.$count(experience);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const [data] = await db.insert(experience).values(parsed.data).returning();
    return created(data, 'Experience created');
  } catch (e) {
    console.error('[Experience] Create failed:', e);
    return err('Failed to create experience', 500);
  }
}
